...
class verify extends Command {
  ...
  execute = async (event: ExecuteEvent) => {
    ...
    if (expected > Date.now()) return event.message.channel.send(new MessageEmbed()
      .setTitle(' ')
      .setDescription(`Your account is less than 14 days old! Try again in __${length}__!`)
      .setColor('#ff0000')); 

    event.message.member?.roles.add(role)
    event.message.channel.send(new MessageEmbed()
      .setTitle(' ')
      .setDescription('You have successfully verified!')
